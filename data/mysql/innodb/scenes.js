  ...
  vary:{ knob:'innodb_doublewrite', base:'ON', order:['DETECT_ONLY','OFF'], alt:{
    'DETECT_ONLY':{
      3:{ act:{ f:'dw', t:'ses', lb:'p:5 가 찢어졌음을 알아낸다' },
          ...
          fact:[['storage/innobase/include/buf0dblwr.h','When --innodb-doublewrite=DETECT_ONLY, page contents are not written to the'],
                ['storage/innobase/include/buf0dblwr.h','this mode is also called as reduced mode. It is called reduced because the']],
          beat:1,
          ops:{ dw:{ set:{ 'p:5':{ tag:'wait', sub:'항목만 있음 · 16바이트' } } },
                ses:{ set:{ '서버 상태':'p:5 손상 확인|red' } } } },
      ...
      7:{ note:'재시도 횟수를 다 쓰면 서버가 죽는다',
          why:'BUF_PAGE_READ_MAX_RETRIES 만큼 다시 읽어 본 뒤 치명 오류로 중단한다. 그 메시지가 innodb_force_recovery 를 안내한다.',
          key:'복구가 <em>시작조차 끝나지 못한다</em>. undo 도, binlog 판정도, 여기까지 오지 못한다.',
          ref:'storage/innobase/buf/buf0buf.cc', sym:'Buf_fetch::read_page',
          fact:[['storage/innobase/buf/buf0buf.cc','" this problem by using innodb_force_recovery. Please "']],
          ops:{ ses:{ set:{ '서버 상태':'기동 실패 · Aborting|red' } } } },

      8:{ note:'binlog 판정 단계는 실행되지 않는다',
          why:'PREPARED 트랜잭션을 찾고 XID 를 대조하는 일은 페이지를 읽을 수 있어야 가능하다. 그 앞에서 멈췄으므로 trx 77 은 판정되지 않은 채 남는다.',
          key:'복구의 단계들은 <em>순서대로 묶여 있다</em>. 물리 복구가 막히면 논리 복구는 시작도 하지 않는다.',
          ref:'sql/binlog/recovery.cc', sym:'Binlog_recovery::recover',
          look:{ undo:true, bin:true },
          ops:{ bin:{ set:{ '판정':'실행되지 않음|red' } },
                ses:{ set:{ 'PREPARED trx':'77  (판정 못 함)|red' } } } },

      9:{ note:'남은 수단은 innodb_force_recovery 다 — 검사를 건너뛴다',
          why:'값을 올리면 손상 검사를 건너뛰고 기동한다. 페이지는 고쳐지지 않는다 — 깨진 바이트를 그대로 들고 연다.',
          key:'그래서 이것은 <em>복구가 아니라 탈출</em>이다. 열린 뒤 할 일은 데이터를 덤프해 새 인스턴스로 옮기는 것뿐이다.',
      ...
      10:{ note:'탐지는 값을 했다 — 무엇을 잃었는지 알려 준 것',
           why:'OFF 였다면 어느 페이지가 왜 깨졌는지도 모른 채 같은 실패를 봤다. 여기서는 p:5 라는 것을 알고 시작한다.',
           key:'DETECT_ONLY 가 사는 자리가 여기다 — 쓰기 비용은 거의 없고, <em>진단은 얻고 복구는 포기</em>한다.',
           ref:'storage/innobase/include/buf0dblwr.h', sym:'Reduced_entry',
           look:{ ibd:true, dw:true },
           beat:1,
           ops:{ ses:{ set:{ '롤백 대상':'수동 이전 필요|red' } } } },
    },
    'OFF':{
      3:{ act:{ f:'ibd', t:'ses', lb:'되살릴 근거가 없다' },
          ...
          fact:[['storage/innobase/include/buf0dblwr.h','/** Equal to FALSEE. In this mode, dblwr is disabled. */']],
          beat:1,
          ops:{ dw:{ del:['p:5'] },
                ses:{ set:{ '서버 상태':'단서 없음|red' } } } },
...
},
{
  ...
  { note:'그런데 부하가 더 세다. 나이가 sync flush 한계를 넘는다',
    ...
    ref:'storage/innobase/include/log0chkp.h', sym:'log_free_check',
    beat:1,
