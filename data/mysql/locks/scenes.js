...
},
{
  ...
  /* 모드를 낮추면 이 장면의 락 수가 처음부터 하나 더 많다 — 테이블 AUTO_INC 락이
     ...
     모드 1 은 평온할 때 2 와 같고 남이 AI 락을 쥐고 있을 때만 0 으로 넘어간다 —
     그 조건부가 이 값의 전부이므로 그 상황을 그린다.
    ...
    '1':{
      1:{ note:'모드 1 은 평온할 때 2 와 같다 — 그런데 지금은 평온하지 않다',
          ...
          fact:[['storage/innobase/handler/ha_innodb.cc','if (ib_table->count_by_mode[LOCK_AUTO_INC]) {'],
                ['storage/innobase/handler/ha_innodb.cc','/* Fall through to old style locking. */']],
          beat:1,
          ops:{ stmt:{ set:{ '단계':'AI 락 대기  (옆 세션이 쥠)|red', '락 수':'1  대기' } },
                rl:{ del:['(비었다)'],
                     add:[{ id:'테이블 AUTO_INC', tag:'wait', sub:'LOCK_AUTO_INC 를 기다림' }] } } },
      2:{ ops:{ stmt:{ set:{ '단계':'삽입 지점 확인', '락 수':'2' } },
                idx:{ span:{ add:[{ id:'ii', from:200, to:300, kind:'ii', lb:'삽입 의도 (200,300)', row:0 }] } },
                rl:{ set:{ '테이블 AUTO_INC':{ tag:'hold', sub:'LOCK_AUTO_INC  ·  문장 끝까지' } },
                     add:[{ id:'갭 (200,300)', tag:'wait', sub:'X,INSERT_INTENTION' }] } } },
      4:{ ops:{ row:{ add:[{ id:'id=25', tag:'x', sub:'c=250  ·  암묵적 X' }] },
                idx:{ add:[{ id:'250', v:250 }] },
                stmt:{ set:{ '단계':'행 삽입', '락 수':'2  (+ 암묵적)' } } } },
      ...
      8:{ look:{ rl:true, stmt:true },
          ...
          ref:'storage/innobase/handler/ha_innodb.cc', sym:'ha_innobase::innobase_lock_autoinc',
          /* fact 를 다시 적는다. 맨 문자열 fact 는 그 스텝의 ref 에 매여 있으므로
             ref 만 바꾸고 물려받으면 엉뚱한 파일에서 찾는다 — 검증이 그것을 잡았다. */
          fact:[['storage/innobase/handler/ha_innodb.cc','static const long AUTOINC_NEW_STYLE_LOCKING = 1;'],
                ['storage/innobase/handler/ha_innodb.cc','if (ib_table->count_by_mode[LOCK_AUTO_INC]) {']],
          beat:1 },
    },
    '0':{
      1:{ act:{ f:'stmt', t:'rl', lb:'테이블에 AUTO_INC 락' },
          ...
          beat:1,
          ops:{ stmt:{ set:{ '단계':'테이블 AI 락 획득', '락 수':'1' } },
                rl:{ del:['(비었다)'],
                     add:[{ id:'테이블 AUTO_INC', tag:'hold', sub:'LOCK_AUTO_INC  ·  문장 끝까지' }] } } },
      2:{ ops:{ stmt:{ set:{ '단계':'삽입 지점 확인', '락 수':'2' } },
                idx:{ span:{ add:[{ id:'ii', from:200, to:300, kind:'ii', lb:'삽입 의도 (200,300)', row:0 }] } },
                rl:{ add:[{ id:'갭 (200,300)', tag:'wait', sub:'X,INSERT_INTENTION' }] } } },
      4:{ ops:{ row:{ add:[{ id:'id=25', tag:'x', sub:'c=250  ·  암묵적 X' }] },
                idx:{ add:[{ id:'250', v:250 }] },
                stmt:{ set:{ '단계':'행 삽입', '락 수':'2  (+ 암묵적)' } } } },
      6:{ ops:{ rl:{ add:[{ id:'uk  c=250', tag:'hold', sub:'LOCK_S  ·  중복 검사' }] },
